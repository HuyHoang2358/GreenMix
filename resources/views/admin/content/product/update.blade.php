                ...
                <div class="mt-5">
                    <div class="form-inline items-start flex-col xl:flex-row mt-5 pt-5 first:mt-0 first:pt-0">
                        ...
                        <div class="w-full mt-3 xl:mt-0 flex-1">
                            <input id="name" name="name" type="text" class="form-control" placeholder="Nhập tên tên dự án" required autofocus value="{{ $product->name }}">
                    ...
                    <div class="form-inline items-start flex-col xl:flex-row mt-5 pt-5 first:mt-0 first:pt-0">
                        ...
                        <div class="w-full mt-3 xl:mt-0 flex-1">
                            <input id="slug" name="slug" type="text" class="form-control" placeholder="Nhập slug sản phẩm" required autofocus value="{{ $product->slug }}">
                            <div class="form-help text-right">Tối đa 0/100 ký tự</div>
                        </div>
                    </div>
                    ...
                    <div class="form-inline items-start flex-col xl:flex-row mt-5 pt-5 first:mt-0 first:pt-0">
                        ...
                        <div class="w-full flex flex-col gap-2">
                            <div class="w-full mt-3 xl:mt-0 flex-1 flex gap-2">
                                ...
                                <input id="image" readonly name="images" type="text" class="form-control flex-1 w-2" placeholder="Tải hình ảnh lên" required autofocus value="{{ $product->images }}">
                            </div>
                            <div>
                                <label for="holder" class="form-label">Hình ảnh xem trước</label>
                                <div id="holder" style="margin-top:15px;">
                                    @if($product->images)
                                        <img class="h-20 w-fit" src="{{ $product->images }}" alt="">
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="form-inline items-start flex-col xl:flex-row mt-5 pt-5 first:mt-0 first:pt-0">
                        ...
                        <div class="w-full mt-3 xl:mt-0 flex-1">
                            <select id="post_id" name="post_id" class="form-control">
                                <option value="" {{ $product->post_id == '' ? 'selected' : '' }}>Không có bài viết nào</option>
                                @foreach($posts as $post)
                                    <option {{ $product->post_id == $post->id ? 'selected' : '' }} value="{{ $post->id }}">Tên: {{ $post->name }} - Tiêu đề: {{ $post->title }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
